...
2. 如果该文件不存在或仍包含 [PLACEHOLDER] 标记，在进行实质性工作前 STOP。说："This plugin needs setup before it can give you useful output. Run /commercial-legal:cold-start-interview — it takes about 10-15 minutes and every command in this plugin depends on it. Without it, outputs will be generic and may not match how your practice actually works." 不要继续使用占位符或默认配置。唯一无需设置即可运行的 skill 是 /commercial-legal:cold-start-interview 本身和任何 --check-integrations 标志。
3. 设置和冷启动访谈 WRITE 到该路径，根据需要创建父目录。
4. 在 plugin 更新后的首次运行时，如果旧缓存路径
   (~/.claude/plugins/cache/claude-for-legal/commercial-legal/<version>/CLAUDE.md 适用于任何版本)
...
[Your Company Name] 是一个 [entity type]。合同团队有 [N] 人。[GC name] 是最终升级点。我们每月处理大约 [N] 份协议，主要是 [vendor / customer / mixed]。我们使用 [CLM system] 进行合同生命周期管理。
...
**律师联系人：** [PLACEHOLDER — Name / team / outside firm / N/A if a lawyer]
...
| 集成 | 状态 | 不可用时的备用方案 |
|---|---|---|
| CLM (Ironclad、Agiloft 等) | [PLACEHOLDER ✓/✗] | 手动记录；续约跟踪器针对本地登记册运行 |
| 电子签名 (DocuSign 等) | [PLACEHOLDER ✓/✗] | 用户在 plugin 外部路由签名 |
| 文档存储 (Drive / SharePoint / Box) | [PLACEHOLDER ✓/✗] | 用户为每次审查直接上传协议 |
| Slack | [PLACEHOLDER ✓/✗] | 警报和利益相关者摘要内联传递而非发布 |
...
**活跃方：** [PLACEHOLDER — sales / purchasing / both — 在冷启动时设置]

*销售方 = 公司销售其产品或服务。我们是供应商。通常是我们的文件。采购方 = 公司从第三方供应商或供应商处购买。我们是客户。通常是他们的文件。答案会改变每个剧本立场——风险偏好、标准和备用条款、审批阈值、责任限制、赔偿方向、IP 所有权、终止权利。*

> 根据此剧本审查或评估合同的 Skills 首先确定公司在哪一方（通常从谁的文件中可以明显看出——如果对手方正在购买你的产品，你是销售方；如果你正在购买他们的产品，你是采购方）。如果不明显，询问。阅读匹配的剧本部分。永远不要将销售方立场应用于采购方合同，反之亦然。
...
**直接限额（费用倍数）：** [PLACEHOLDER — 例如，"12 months fees paid or payable"]

**间接/后果性损害赔偿：** [PLACEHOLDER — excluded / capped at [X] / uncapped / mirrors direct]

**可接受的例外（限额之上）：** [PLACEHOLDER — 例如，"Gross negligence, breach of confidentiality, IP indemnity, data breach"]

**我们接受的限额基数定义：** [PLACEHOLDER — 例如，"fees paid in the 12 months preceding the claim" vs. "fees payable under the current order form"——选择你将接受的定义并标记模糊语言]

**可接受的备用方案：**
- [PLACEHOLDER]

**永远不要接受：**
- [PLACEHOLDER — 例如，"Uncapped indirect damages", "cap base tied to last 3 months of fees"]

#### 赔偿

**标准立场：** [PLACEHOLDER — 例如，"We indemnify for IP infringement claims arising from the service; customer indemnifies for its data and use"]
...
**标准立场：** [PLACEHOLDER — 例如，"Our DPA as processor; customer's DPA accepted with redlines"]
...
**标准立场：** [PLACEHOLDER — 例如，"Annual term, auto-renewing, 30-day notice to cancel"]

**可接受的备用方案：**
- [PLACEHOLDER]

**永远不要接受：**
- [PLACEHOLDER — 例如，"Termination for convenience during paid term"]

#### 适用法律和地点

**首选：** [PLACEHOLDER — 例如，"Delaware, our home jurisdiction"]
**可接受：** [PLACEHOLDER]
**升级：** [PLACEHOLDER]
**从不：** [PLACEHOLDER]
...
[PLACEHOLDER — 我们销售时的交易破坏者。每次销售方审查首先检查这个。]
...
**直接限额（费用倍数）：** [PLACEHOLDER — 例如，"Vendor cap at 12 months fees paid or payable; higher for data breach and IP indemnity"]

**间接/后果性损害赔偿：** [PLACEHOLDER — excluded / capped at [X] / uncapped from vendor / mirrors direct]

**我们要求的例外（限额之上）：** [PLACEHOLDER — 例如，"Gross negligence, breach of confidentiality, IP indemnity, data breach"]

**我们接受的限额基数定义：** [PLACEHOLDER — 例如，"fees paid in the 12 months preceding the claim"——选择你将接受的定义；"fees paid in prior 3 months" 或 "fees under current order form only" 是常见的供应商有利定义，要拒绝]

**可接受的备用方案：**
- [PLACEHOLDER]

**永远不要接受：**
- [PLACEHOLDER — 例如，"Vendor liability capped at fees paid in prior 3 months", "cap base undefined"]

#### 赔偿

**标准立场：** [PLACEHOLDER — 例如，"Vendor indemnifies for IP infringement and data breach; we indemnify for our data"]
...
**标准立场：** [PLACEHOLDER — 例如，"Vendor signs our DPA as processor"]

**要求：**
- [PLACEHOLDER — 例如，"SOC 2 Type II for any vendor touching customer data"]
...
**标准立场：** [PLACEHOLDER — 例如，"Termination for convenience on 30 days' notice; auto-renewal only with 30-day cancel window"]

**可接受的备用方案：**
- [PLACEHOLDER]

**永远不要接受：**
- [PLACEHOLDER — 例如，"Multi-year lock-in with no termination rights"]

#### 适用法律和地点

**首选：** [PLACEHOLDER — 例如，"Delaware, New York, California"]
**可接受：** [PLACEHOLDER]
**升级：** [PLACEHOLDER]
**从不：** [PLACEHOLDER]
...
[PLACEHOLDER — 我们购买时的交易破坏者。每次采购方审查首先检查这个。]
...
**无论美元价值如何的自动升级：**
- [PLACEHOLDER — 例如，"Unlimited liability, IP assignment to vendor, anything on a Never list above"]
...
**工作产品去向：** [PLACEHOLDER — CLM、Drive 文件夹、Slack 频道]

**续约警报发送到：** [PLACEHOLDER — Slack 频道或电子邮件]
...
从面向外部的可交付成果中删除标题（转发到法律外部的利益相关者摘要、面向对手方的红线）——请参阅特定 skill 的说明。确认你的司法管辖区和事项的正确标记。
...
**在任何引用权威的 skill 之前进行飞行前检查。** 测试研究连接器（Westlaw、CourtListener 或法规/监管机构 MCP）是否实际响应，而不仅仅是配置。如果没有，在审阅者注释的 **Sources:** 行中记录它（请参阅 `## Outputs`）——例如，`not connected — cites from training knowledge, verify before relying`。不要在标题上方发出独立横幅。审阅者注释是此信号存在的唯一地方；每个引用的 `[model knowledge — verify]` 标记保持内联。
...
**双重严重性。** 商业合同发现有两个轴：
- **法律风险：** 🔴 Blocking / 🟠 High / 🟡 Medium / 🟢 Low——我们会被起诉、罚款或制裁吗？
- **业务摩擦：** 🔴 Blocks deals / 🟠 Slows deals / 🟡 Confuses customers / 🟢 Invisible——这会让我们损失收入、信任或时间吗？

🟢 法律风险和 🔴 业务摩擦的条款（法律上没问题但读起来像是肯定授予并阻止注册的保密条款）应该在发现登记册中显示为 🔴——因为阅读审查的人关心两者。法律风险列告诉律师这不是责任问题。业务摩擦列告诉企业为什么它仍然值得修复。
...
**验证日志。** 当你或用户验证标记项时——根据主要来源确认引用、根据本地规则检查截止日期、根据当前法规验证阈值——记录它，以便下一个人不会重新验证。向 `~/.claude/plugins/config/claude-for-legal/commercial-legal/verification-log.md` 写一行：
...
当用户在此 plugin 的执业领域提出问题时——不仅仅是当他们调用 skill 时——首先阅读 `~/.claude/plugins/config/claude-for-legal/commercial-legal/CLAUDE.md`（和 `~/.claude/plugins/config/claude-for-legal/company-profile.md`）的执业档案，并应用它。如果它已填充，作为配置的助手回答：
...
- 当从问题中得出行动时提供决策树
- 如果有结构化 skill 会做得更好，建议一个："This is a quick answer. If you want the full framework, run `/commercial-legal:[relevant skill]`."

如果执业档案未填充："I can give you a general answer, but this plugin gives much better answers once it's configured to your practice — run `/commercial-legal:cold-start-interview` (2-minute quick start or 10-minute full setup)." 然后无论如何给出一般答案，标记为未配置。
...
*仅与多客户执业相关（私人执业——独立、小律所、大律所）。如果你在内部只有一个客户，此部分关闭，以下内容均不适用——skills 自动使用执业级上下文，而 `/commercial-legal:matter-workspace` 不是你需要的东西。*
...
当 skill 不知道哪个事项活跃且工作区启用时，它会询问："Which matter? Or practice-level context?" 在进行实质性工作之前。使用 `/commercial-legal:matter-workspace new | list | switch | close | none` 管理事项。
...
closing_action: "[PLACEHOLDER — 由冷启动访谈设置。在每个 NDA 分流输出末尾附加什么，例如，'Forward this output and the NDA to your contracts manager.']"
