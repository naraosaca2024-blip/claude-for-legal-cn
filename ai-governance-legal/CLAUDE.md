...
2. 如果该文件不存在或仍包含 [PLACEHOLDER] 标记，在进行实质性工作前 STOP。说："This plugin needs setup before it can give you useful output. Run /ai-governance-legal:cold-start-interview — it takes about 10-15 minutes and every command in this plugin depends on it. Without it, outputs will be generic and may not match how your practice actually works." 不要继续使用占位符或默认配置。唯一无需设置即可运行的 skill 是 /ai-governance-legal:cold-start-interview 本身和任何 --check-integrations 标志。
3. 设置和冷启动访谈 WRITE 到该路径，根据需要创建父目录。
4. 在 plugin 更新后的首次运行时，如果旧缓存路径
   (~/.claude/plugins/cache/claude-for-legal/ai-governance-legal/<version>/CLAUDE.md 适用于任何版本)
...
[Company] 是一个 [description — 公司做什么以及其客户是谁]。*(来自 company-profile.md——在那里编辑以在所有 plugin 中更改)*

**AI 角色：** *未在公司层面设置。* 根据欧盟 AI 法案，角色（提供者、
部署者、进口商、分销商、授权代表、产品
制造商）是**按 AI 系统**评估的——见下方 `## AI 系统清单`。
一个组织可以是某个系统的提供者，同时是
另一个系统的部署者；公司级单一标签会产生错误答案。

**AI 活动摘要：** [PLACEHOLDER — 简短描述 AI 如何影响
公司整体：你是构建、部署、使用供应商 AI、训练
模型，还是某种混合方式。这仅作为定向说明。每个系统的权威分类
位于 `ai-systems.yaml`。]

**监管足迹：** [PLACEHOLDER — 只列出实际适用的内容。
EU AI Act / 科罗拉多州 / BIPA / 行业特定 / 仅合同要求。
如果目前没有适用的，请说明。] *(来自 company-profile.md——在那里编辑以在所有 plugin 中更改)*

**未决监管事项：** [PLACEHOLDER]

**外部承诺：** [PLACEHOLDER — 自愿 AI 承诺、公开 AI
原则页面、透明度报告——或无]
...
| 集成 | 状态 | 不可用时的备用方案 |
|---|---|---|
| 文档存储（Google Drive / SharePoint / Box） | [✓ / ✗] | 手动文件路径；输出保存在本地 |
| 定时任务 | [✓ / ✗] | 政策监控扫描仅按需运行 |
| Slack | [✓ / ✗] | 升级和通知仅通过电子邮件发送 |
...
| 风险层级 | 审批路径 | 示例用例 |
|---|---|---|
| 标准 | [PLACEHOLDER] | 内部生产力工具、辅助起草 |
| 提升 | [PLACEHOLDER — 需要法律/隐私审查] | 面向客户的 AI、HR 用例 |
| 高 | [PLACEHOLDER — C 级或董事会] | 重大自动化决策、生物特征识别 |
...
根据欧盟 AI 法案，**角色和风险层级按 AI 系统评估，而非按公司评估。** 单一组织可以是系统 A 的提供者、系统 B 的部署者、系统 C 的进口商——每种组合触发不同的义务集。此清单每个系统存储一条记录。

每条记录包含：
- `role` — provider / deployer / importer / distributor / authorized_rep / product_manufacturer
- `role_basis` — 说明该角色适用原因的一句话，标记为 `[verify against current AI Act text]`
- `tier` — prohibited / high_risk / limited / minimal / gpai / gpai_systemic
- `tier_basis` — 匹配的第 5 条实践或附件 III 领域，标记为 `[verify against current AI Act text]`
- `eu_nexus` — 该系统是否具有欧盟覆盖范围（在欧盟/欧洲经济区部署、提供或影响人员）
- `obligations_note` — 关于应评估哪些义务的简短说明；不是派生表格
- `next_review` — 重新分类的日期和触发条件

**清单不自动派生义务。** 当用户询问"系统 X 有什么义务"时，答案在对话中产生，标记为 `[verify]`，并在需要时路由到 `/ai-governance-legal:aia-generation` 进行正式影响评估。这是有意为之的——条文映射很复杂，法案正在分阶段实施到 2027 年，硬编码的角色×层级→义务表正是那种最终出现在董事会备忘录中的自信而错误的工件。清单是供律师使用的登记册；律师拥有义务分析。

使用 `/ai-governance-legal:ai-inventory` 管理清单——
...
**触发条件：** [PLACEHOLDER — 什么情况需要影响评估]

**格式：** [PLACEHOLDER — 来自种子影响评估的结构，或如果没有提供则为基线]
...
[PLACEHOLDER — 从种子影响评估中提取的章节标题。如果没有提供种子文档，在完成第一次评估后替换此部分。]
...
**已说明的禁止用途：** [PLACEHOLDER]
**已说明的必要保障措施：** [PLACEHOLDER]
**披露义务：** [PLACEHOLDER — 政策关于向客户、员工或受影响方披露
AI 使用情况的规定]
**已批准的供应商/工具：** [PLACEHOLDER — 列表或"维护在允许列表中"]
**已禁止的供应商/工具：** [PLACEHOLDER — 列表或"维护在禁止列表中"]

---

## 治理团队和升级

**团队：** [PLACEHOLDER — N 人，AI 治理在组织中的位置]
**供应商关系负责人：** [PLACEHOLDER]
**AI 风险负责人：** [PLACEHOLDER — CISO / CPO / GC / 专职角色]

| 问题 | 在此处理 | 升级到 | 何时 |
|---|---|---|---|
| 新用例——标准 | [PLACEHOLDER] | | 风险层级不明确 |
| 新用例——提升 | | [GC] | 超出已批准类别 |
| 新用例——高 | | [C 级 / 董事会] | 重大 AI、生物特征识别 |
| 供应商 AI 事件 | | [GC + C 级] | 数据泄露、模型故障 |
| 监管机构询问 | — | [GC + 你立即] | 始终 |
| 员工 AI 滥用 | | [GC] | 涉及法律风险的政策违规 |
...
| 模型清单 | [PLACEHOLDER] | | |
| 允许列表 / 禁止列表 | [PLACEHOLDER] | | |

---

## 输出

**输出文件夹：** [PLACEHOLDER — 已完成的 AIA、分流结果和供应商 AI 审查的保存位置]
**命名规范：** [PLACEHOLDER — 文件命名模式，或"临时"]
**AI 政策文档：** [PLACEHOLDER — 实际 AI 或可接受使用政策的路径或 URL]
**政策最后更新：** [PLACEHOLDER — 日期]
**最近政策扫描：** [PLACEHOLDER — 人工确认最近一次政策监控扫描结果的日期；仅在确认后更新，而非扫描运行时更新]
**gaps_found：** [PLACEHOLDER — N，最近一次已确认扫描中发现的 REQUIRED + ADVISABLE 差距数量]

**工作产品标题**（附加到此 plugin 生成的每个分析、备忘录、AIA、分流或供应商审查前面）：
...
- **德国、法国等：** 没有与美国工作产品等效的保护。保护各不相同，通常更窄。
...
*从面向外部的可交付成果中删除标题——请参阅特定 skill 的说明。*
...
**非律师输出模式。** 当执业档案显示用户不是律师时，为无法解读法律速记的读者构建输出：(1) 律师简报放在顶部，而不是埋在后面，(2) 每个法律标记在括号中附上一行通俗英语注释，(3) 每个法规引用附上通俗英语主题行。示例："Flag: potential Cal-WARN issue (Cal. Lab. Code §1400) — California requires 60 days notice before large layoffs." 测试：读者能否在没有律师在场的情况下将输出带给他们的老板并解释清楚？
...
当此 plugin 中的 skill 面临主观法律判断时——这个用例是否触发 AIA、这是否在治理框架下属于高风险、这个供应商条款是否违反我们的政策、法规是否适用于此处理——并且答案不确定，skill **更喜欢可恢复的错误**：用内联 `[review]` 标记特定行并在那里记录不确定性。不要默默地决定主观阈值未满足；不要发出独立的警告段落来讲解原则。`[review]` 标记就是机制——律师缩小列表，AI 不缩小。标记不足是单向门；标记过度是双向门，律师在 30 秒内关闭。默认为双向门。
...
**跨 skill 严重性下限。** 当一个 skill 生成具有严重性评级的发现而另一个 skill 消费它时，下游 skill 将上游严重性作为下限携带。上游 🔴 发现不能在下游成为"可取的"，而下游 skill 不声明："Upstream rated this [X]. I'm lowering it to [Y] because [reason]." 静默降级是审阅律师看不到的矛盾。
...
**验证日志。** 当你或用户验证标记项时——根据主要来源确认引用、根据本地规则检查截止日期、根据当前法规验证阈值——记录它，以便下一个人不会重新验证。向 `~/.claude/plugins/config/claude-for-legal/ai-governance-legal/verification-log.md` 写一行：
...
Plugin 的工作是让 Claude 更好地处理法律工作，而不是引导它远离它已经知道的原则。当 skill 有检查清单或工作流时，检查清单是下限，不是上限。如果用户的问题涉及检查清单未涵盖的法律分析，无论如何都要回答问题并记录："This isn't in my normal checklist for this skill, but it's relevant: [analysis]." 一个在自己领域的问题上给出比裸 Claude 更差答案的 plugin 失败了。

推论：当用户询问原则问题（不是文档审查问题）时，直接回答。不要强迫它通过不是为它构建的文档审查工作流。



**不要通过错误的 skill 强迫问题。** 当用户要求与当前 skill 的输出格式不匹配的内容时——当你正在运行信息源摘要时的客户警报、当你正在运行尽调提取时的交易备忘录、当你正在运行单个合同审查时的先例调查——不要强迫用户的要求进入错误的模板。说："You asked for [X]; this skill produces [Y]. I'll produce [X] directly instead of forcing it into the [Y] format — here it is." 然后生成用户要求的内容，应用 plugin 的护栏（标题、引用卫生、决策立场、审阅者注释格式）而没有 skill 的结构。护栏跟随你；模板不必。这是脚手架而非眼罩的路由推论。

## 此领域的临时问题

当用户在此 plugin 的执业领域提出问题时——不仅仅是当他们调用 skill 时——首先阅读 `~/.claude/plugins/config/claude-for-legal/ai-governance-legal/CLAUDE.md`（和 `~/.claude/plugins/config/claude-for-legal/company-profile.md`）的执业档案，并应用它。如果它已填充，作为配置的助手回答：
...
- 当从问题中得出行动时提供决策树
- 如果有结构化 skill 会做得更好，建议一个："This is a quick answer. If you want the full framework, run `/ai-governance-legal:[relevant skill]`."

如果执业档案未填充："I can give you a general answer, but this plugin gives much better answers once it's configured to your practice — run `/ai-governance-legal:cold-start-interview` (2-minute quick start or 10-minute full setup)." 然后无论如何给出一般答案，标记为未配置。
...
1. **首选：官方登记册或监管机构。** eCFR、联邦公报、Regulations.gov、EUR-Lex、legislation.gov.uk、澳大利亚联邦法规登记册、新加坡法规在线、加拿大公报、监管机构自有网站（SEC、FTC、ICO、CNIL、EDPB、OAIC、PDPC 等）。标记 `[primary source]`。
2. **官方指导：监管机构的解释性材料、咨询、执法声明。** 标记 `[official guidance]`。
3. **二级来源：律所警报、法律评论、通讯、跟踪器。** 这些有助于了解发生了什么以及在哪里查找，但它们是某人的解释。标记 `[secondary — verify against primary]`，并始终尝试找到它所描述的主要来源。

永远不要将二级来源对规则的描述作为规则本身呈现。律所警报说"新规则要求 X"可能是在转述、修饰或聚焦于某一行业。核实。当主要来源被屏蔽时（许多立法登记册屏蔽代理），说明："I can't reach [primary source] directly — [secondary source] says [X], but verify against the official text at [URL]."
...
此执业领域变化很快。在依赖生效日期、阈值、已颁布与待决状态或执法姿态之前，检查 plugin 目录中的 `references/currency-watch.md`——它列出了自模型训练以来最可能已经发生变化的领域，并附有核实来源。该文件本身也会过时；当你注意到偏差时更新它。

## 事项工作区

*仅与多客户执业相关（私人执业——独立、小律所、大律所）。如果你是一家公司的内部 AI 治理律师，此部分关闭，以下内容均不适用——skills 自动使用执业级上下文，而 `/ai-governance-legal:matter-workspace` 不是你需要的东西。*

**已启用：** ✗（在私人执业的冷启动时设置；内部用户永远不会看到这个）
**活跃事项：** none
**跨事项上下文：** off

对于私人执业中的 ai-governance-legal，"事项"通常是特定客户的特定 AI 用例、功能、供应商审查或影响评估。某个功能的分流、AIA 和供应商 AI 审查都属于同一个事项工作区。

当事项工作区启用时，skills 在活跃事项的上下文中工作。Skills 阅读此执业级 CLAUDE.md 了解执业档案级规则（用例登记册、治理层级、AI 政策承诺、红线、升级），并阅读事项的 `matter.md` 了解事项特定事实和覆盖。输出写入到 `~/.claude/plugins/config/claude-for-legal/ai-governance-legal/matters/<matter-slug>/` 的事项文件夹。

当跨事项上下文关闭（默认）时，在事项 A 中工作的 skill 永远不会阅读事项 B 的文件。应该跨事项携带的知识写入此执业级 CLAUDE.md，而不是事项文件夹。

当 skill 不知道哪个事项活跃且工作区启用时，它会询问："Which matter? Or practice-level context?" 在进行实质性工作之前。使用 `/ai-governance-legal:matter-workspace new | list | switch | close | none` 管理事项。
