...
2. 如果该文件不存在或仍包含 [PLACEHOLDER] 标记，在进行实质性工作前 STOP。说："This plugin needs setup before it can give you useful output. Run /regulatory-legal:cold-start-interview — it takes about 10-15 minutes and every command in this plugin depends on it. Without it, outputs will be generic and may not match how your practice actually works." 不要继续使用占位符或默认配置。唯一无需设置即可运行的 skill 是 /regulatory-legal:cold-start-interview 本身和任何 --check-integrations 标志。
...
**律师联系人：** [PLACEHOLDER — 姓名 / 团队 / 外部律所 / N/A；如为非律师则填写]

*Skills 读取此部分以选择工作产品标题，并决定是否对重要操作设置门槛（见下方 `## 输出` 和各 skill 的门槛设置）。*
...
| 集成 | 状态 | 不可用时的备用方案 |
|---|---|---|
| 监管信息源（付费订阅） | [✓ / ✗] | 免费联邦公报 API + 用户粘贴的警报；无增强层 |
| 文档存储（Google Drive、SharePoint、Box） | [✓ / ✗] | 从本地路径索引的政策库 |
| Slack | [✓ / ✗] | 摘要仅作为文件输出；无频道内警报 |

*联邦公报 API 是免费的公共端点——始终可用，无需 MCP 连接器。*
...
**位置：** [PLACEHOLDER — Drive 文件夹、SharePoint、Confluence]
...
**始终重要（立即行动）：**
- [PLACEHOLDER — 例如，"有截止日期的新义务"、"本行业的执法行动"]

**值得审查（评估并决定）：**
- [PLACEHOLDER — 例如，"拟议规则"、"指导文件"、"针对竞争对手的执法行动"]

**仅供参考（记录，无需行动）：**
- [PLACEHOLDER — 例如，"委员的演讲"、"学术评论"]

---

## 差距应对流程

**谁负责对监管变化进行分流：** [PLACEHOLDER]
**谁负责政策更新：** [PLACEHOLDER]
**差距如何跟踪：** [PLACEHOLDER — 工单系统、电子表格等]
**重大差距的升级：** [PLACEHOLDER]
...
**付费监管信息源：** [PLACEHOLDER — 提供商、订阅、警报]
**CourtListener：** [PLACEHOLDER]
**直接监管机构信息源：** [PLACEHOLDER — RSS、邮件列表]
...
此 plugin 的 skills 生成分析、政策差异、差距报告和信息源摘要。附加在每个输出前的**工作产品标题**取决于 `## 谁在使用这个` 中的角色：

- 如果角色是**律师/法律专业人士**：`PRIVILEGED & CONFIDENTIAL — ATTORNEY WORK PRODUCT — PREPARED AT THE DIRECTION OF COUNSEL`
- 如果角色是**非律师**（任何类型）：`RESEARCH NOTES — NOT LEGAL ADVICE — REVIEW WITH A LICENSED ATTORNEY BEFORE ACTING`
...
对外部可交付成果（公开评论提交、机构回复、面向客户的备忘录）关闭标题——请参阅特定 skill 的说明。在分发前向律师确认你的司法管辖区和事项的正确标记。仅凭标记不能创建特权。
...
**验证日志。** 当你或用户验证标记项时——根据主要来源确认引用、根据本地规则检查截止日期、根据当前法规验证阈值——记录它，以便下一个人不会重新验证。向 `~/.claude/plugins/config/claude-for-legal/regulatory-legal/verification-log.md` 写一行：
...
当用户在此 plugin 的执业领域提出问题时——不仅仅是当他们调用 skill 时——首先阅读 `~/.claude/plugins/config/claude-for-legal/regulatory-legal/CLAUDE.md`（和 `~/.claude/plugins/config/claude-for-legal/company-profile.md`）的执业档案，并应用它。如果它已填充，作为配置的助手回答：
...
- 当从问题中得出行动时提供决策树
- 如果有结构化 skill 会做得更好，建议一个："This is a quick answer. If you want the full framework, run `/regulatory-legal:[relevant skill]`."

如果执业档案未填充："I can give you a general answer, but this plugin gives much better answers once it's configured to your practice — run `/regulatory-legal:cold-start-interview` (2-minute quick start or 10-minute full setup)." 然后无论如何给出一般答案，标记为未配置。
...
*仅与多客户执业相关（私人执业——独立、小律所、大律所）。如果你是一家公司的内部监管律师，此部分关闭，以下内容均不适用——skills 自动使用执业级上下文，而 `/regulatory-legal:matter-workspace` 不是你需要的东西。*

**已启用：** ✗（在私人执业的冷启动时设置；内部用户永远不会看到这个）
**活跃事项：** none
**跨事项上下文：** off

对于私人执业中的 regulatory-legal，"事项"通常是向一个客户提建议的特定监管变化、公开评论期、差距整改项目或机构询问。信息源监控默认在执业级别运行。

当事项工作区启用时，skills 在活跃事项的上下文中工作。Skills 阅读此执业级 CLAUDE.md 了解执业档案级规则（关注的监管机构、政策库、重要性阈值、升级），并阅读事项的 `matter.md` 了解事项特定事实和覆盖。输出写入到 `~/.claude/plugins/config/claude-for-legal/regulatory-legal/matters/<matter-slug>/` 的事项文件夹。

当跨事项上下文关闭（默认）时，在事项 A 中工作的 skill 永远不会阅读事项 B 的文件。应该跨事项携带的知识写入此执业级 CLAUDE.md，而不是事项文件夹。

当 skill 不知道哪个事项活跃且工作区启用时，它会询问："Which matter? Or practice-level context?" 在进行实质性工作之前。使用 `/regulatory-legal:matter-workspace new | list | switch | close | none` 管理事项。
